...
> Пример передачи значения переменной из функции:

    ./script1.sh
    !#/bin/bash
    function return_entered_username() {
        if [[ "$#" -eq 1 ]]; then
            # переменная username_func
            # становится ссылкой на переменную $1
            # изменяя переменную username_func
            # мы изменяем $1
        ...
        while true; do
            ...
            # echo ${username}  # еще один способ передачи значений
            return 0            # код возврата функции
        done
    }
    ...
    # первый вариант:
    # вызов функции return_entered_username
    # с передачей значения "по ссылке"
    # через переменную username
    return_entered_username username
    # либо второй вариант:
    username=$(return_entered_username)
    # но при втором варианте любой иной вызов echo или printf
    # в скрипте повлечет за собой
    # передачу всех данных ч/з поток "stdout" как массива
